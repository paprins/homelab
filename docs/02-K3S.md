...
In this homelab we use **1 server + 2 agents**. This is not highly available (if the server dies, you cannot manage the cluster), but it is simple and works well for learning.
...
| `--disable servicelb` | Do not install the built-in ServiceLB (Klipper). We will use [MetalLB](03-METALLB.md) instead, which is more flexible. |
| `--disable traefik` | Do not install the built-in Traefik. We will install [Traefik via Helm](04-TRAEFIK.md) instead, which gives us control over the version and configuration. |
...
Since we disabled `servicelb` and `traefik`, those are not present. We will install their replacements in the next guides.
